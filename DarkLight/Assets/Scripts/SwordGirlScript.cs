...
public class SwordGirlScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //假设玩家处于攻击状态，且攻击已经完毕，则返回到Idle状态
          if (!mStateInfo.IsName(IdleState) && mStateInfo.normalizedTime >= 1F)//动画播放的程度
        {
    ...
    void Event1()
    {
        ...
        Instantiate(eff1, transform.position +( transform.forward*1.5f+transform.right*   0.5f), Quaternion.identity);
    }
}